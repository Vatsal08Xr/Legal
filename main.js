...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      setTimeout(() => {
        ...
        form.reset();
        if (selectEl) selectEl.style.color = '#86868B';
      }, 3000);
    });
  }
  ...
  if (window.innerWidth > 768) {
    document.querySelectorAll('.service-card, .pricing-card, .team-card, .advantage-card, .testimonial-card').forEach(card => {
  ...
  // ──────────────────────────────────────────────────
  document.querySelectorAll('.services__grid, .pricing__grid, .team__grid, .advantage__grid, .testimonials__grid').forEach(grid => {
